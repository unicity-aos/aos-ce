...
NIGHTLY = f"2026.1.3-nightly.20260717.g{COMMIT}"
...
class NightlyVersionTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        (self.root / "distros/community/unicity-ce/Distro.toml").write_text(
            '[distro]\npretty-name = "Unicity CE 2026.1.3 (Genesis)"\nversion = "2026.1.3"\nrelease-date = "2026-07-10"\n',
            encoding="utf-8",
        )
    ...
    def test_stage_updates_only_product_identity(self) -> None:
        ...
        self.assertEqual(
            compatibility,
            compatibility_before.replace('version = "2026.1.3"', f'version = "{NIGHTLY}"', 1),
        )
    ...
    def test_rejects_noncanonical_inputs(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "nightly version"):
            nightly_version.stage(self.root, "2026.1.3-rc.1")
