(module
  ...
  (func (export "_start")
    ...
    (local.set $fd
      (call $open (i32.const 0) (local.get $path-length) (i32.const 0)))
    (loop $read-loop
      (local.set $read-length
        (call $read (local.get $fd) (i32.const 4096) (i32.const 4096)))
      (if (i32.gt_s (local.get $read-length) (i32.const 0))
        (then
          (drop (call $write
            (i32.const 1)
            (i32.const 4096)
            (local.get $read-length)))
          (br $read-loop))))
